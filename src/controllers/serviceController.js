...
export const getAllServices = async (req, res) => {
  try {
    ...
    const total = await Service.countDocuments({ is_deleted: false });

    res.json({ services, total, page, totalPages: Math.ceil(total / limit) });
  } catch (error) {
  ...
  try {
    ...
    if (priceLines.length > 0) {
      return res.status(400).json({
        message: "Không thể cập nhật dịch vụ vì có các dòng giá liên quan.",
      });
    }
...
export const deleteService = async (req, res) => {
  try {
    ...
    if (priceLines.length > 0) {
      return res.status(400).json({
        message: "Không thể xóa dịch vụ vì có các dòng giá liên quan.",
      });
    }
    ...
    let priceHeaders = await PriceHeader.findOne({
      service_id: serviceId,
      is_deleted: false,
      is_active: true,
    });
    if (priceHeaders) {
      return res.status(400).json({
        message:
          "Dịch vụ này đang được sử dụng trong bảng giá: " +
          priceHeaders.price_list_name,
      });
    }
