...
export const getCustomerRankById = async (req, res) => {
  ...
  try {
    ...
    if (!customerRank) {
      return res.status(404).json({ message: "Không tìm thấy hạng khách hàng" });
    }
...
export const addCustomerRank = async (req, res) => {
  ...
  try {
    ...
    if (existingRank) {
      return res.status(400).json({ message: "Tên hạng khách hàng đã tồn tại" });
    }
    ...
    res
      .status(201)
      .json({ message: "Hạng khách hàng mới đã được thêm", customerRank });
  } catch (err) {
...
export const updateCustomerRank = async (req, res) => {
  ...
  try {
    ...
    if (rank_name) {
      const existed = await CustomerRank.findOne({
        rank_name,
        _id: { $ne: rankId },
      });
      if (existed) return res.status(400).json({ message: "Tên hạng đã tồn tại" });
      customerRank.rank_name = rank_name;
    }
    ...
    await customerRank.save();
    res.json({ message: "Cập nhật hạng khách hàng thành công", customerRank });
  } catch (err) {
...
export const deleteCustomerRank = async (req, res) => {
  ...
  try {
    ...
    await customerRank.save();
    res.json({ message: "Hạng khách hàng đã được xóa mềm", customerRank });
  } catch (err) {
