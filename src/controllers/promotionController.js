...
export const addPromotionHeader = async (req, res) => {
  ...
  try {
    ...
    res.status(201).json({
      message: "Chương trình khuyến mãi mới đã được thêm",
      promotionHeader,
    });
  } catch (error) {
...
export const updatePromotionHeader = async (req, res) => {
  ...
  if (end_date <= Date.now()) {
    return res
      .status(400)
      .json({ message: "Ngày kết thúc phải sau ngày hiện tại" });
  }
...
export const updatePromotionDetail = async (req,res) =>{
  ...
  try {
    ...
    if (invoice.length > 0) {
      return res
        .status(400)
        .json({ message: "Khuyến mãi này đang được sử dụng trong hóa đơn" });
    }
